...
def get_FT_params(parser):
    '''
        The parameters of model FT and WarmupAndFix
    '''
    parser.add_argument("--FT_peft_type", type=str, default='None', choices=['None','PromptTuning','LoRA'], help="The PEFT type")
    parser.add_argument("--FT_fix_old_classifier", default=False, type=bool, help="if fix the old classifiers")
    parser.add_argument("--FT_fix_encoder", default=False, type=bool, help="if fix the encoder")
    parser.add_argument("--FT_warmup_epoch_before_fix_encoder", type=int, default=1, help="The number of fine-tuning epoch before fixing the encoder (only for the first task). NOTE: Only used when FT_fix_encoder is True")
    parser.add_argument("--FT_warmup_target", type=str, default='causal-lm', choices=['causal-lm','ce-loss'], help="The training target during warmup epochs (only for the first task). NOTE: Only used when FT_fix_encoder is True")
    parser.add_argument("--FT_preallocated_classifier", type=bool, default=False, help="If pre-allocating classifiers for optimization?")
    parser.add_argument("--FT_use_prototype_for_prediction", type=bool, default=False, help="If using prototype as classifier weights")
...
class FT(BaseLearner):
    '''
        Training a model FTuentially to a number of tasks
    '''
    def __init__(self, params, CL_dataset, accelerator): 
        super().__init__(params, CL_dataset, accelerator)
        assert params.classifier in ['None','Linear','CosineLinear'], 'NotImplemented for classifier %s and model %s'%(params.classifier,'FT')
        assert params.il_mode in ['CIL','TIL'], 'NotImplemented for il mode %s and model %s'%(params.il_mode,'FT')
        ...
        if params.FT_use_prototype_for_prediction:
    ...
    def build_optimizer(self):
        ...
        if self.params.classifier!='None' and (not (self.params.FT_fix_encoder and self.params.FT_warmup_epoch_before_fix_encoder==0)):
        ...
        elif self.params.classifier!='None' and (self.params.FT_fix_encoder and self.params.FT_warmup_epoch_before_fix_encoder==0):
    ...
    def begin_task(self, task_id):
        ...
        if task_id>0 and self.params.classifier!='None' and self.params.FT_fix_old_classifier:
    ...
    def train_epochs(self, task_id):
        ...
        if task_id==0 and self.params.FT_fix_encoder:
            total_epochs += self.params.FT_warmup_epoch_before_fix_encoder
    ...
    def begin_epoch(self, task_id, epoch_id):
        ...
        # Fix a component by setting its learning rate as zero
        if task_id==0 and self.params.FT_fix_encoder and epoch_id==self.params.FT_warmup_epoch_before_fix_encoder:
    ...
    def observe_batch(self, task_id, epoch_id, lm_input):
        ...
        if self.params.classifier == 'None' or \
            (task_id==0 and self.params.FT_fix_encoder and epoch_id<self.params.FT_warmup_epoch_before_fix_encoder and self.params.FT_warmup_target=='causal-lm'):
        ...
        # Training with Cross-Entropy Loss
            ...
            ((not self.params.FT_use_prototype_for_prediction) or \
            (self.params.FT_use_prototype_for_prediction and task_id==0 and self.params.FT_fix_encoder and epoch_id<self.params.FT_warmup_epoch_before_fix_encoder and self.params.FT_warmup_target=='ce-loss')):
            ...
            if self.params.il_mode == 'CIL':
                ...
                if self.params.FT_preallocated_classifier:
        ...
        elif self.params.FT_use_prototype_for_prediction:
            # Skip training
            return None
        else:
            raise NotImplementedError()
    ...
    def end_epoch(self, task_id, epoch_id):
        ...
        if self.params.FT_use_prototype_for_prediction and \
            (not (task_id==0 and self.params.FT_fix_encoder and epoch_id<self.params.FT_warmup_epoch_before_fix_encoder)):
    ...
    def evaluate_current_task(self,
        ...
        if classifier_list is None:
            ...
            # NOTE: When not using classifier, the FT model does not need (benefit from) task identity 
